...
Sprint 1 has two sales owners: the user and one colleague. Use a controlled owner list rather than free text so assignments and reports remain consistent. The two display names still need to be supplied before implementation. Do not build a full multi-user permission system into this Sprint unless both people require separate logins.
...
- What are the display names of the two sales owners, and do they need separate login accounts?
