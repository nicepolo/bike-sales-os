...
def seed_customers(app):
    ...
    with app.app_context():
        db.session.add_all([
            Customer(id=1, name="Polo 逾期", channel="B2C", status="詢問中", sales_owner="Polo", next_action="電話", next_action_due_date=today - timedelta(days=1)),
            Customer(id=2, name="Polo 今日", channel="B2C", status="預約試騎", sales_owner="Polo", next_action="確認試騎", next_action_due_date=today),
            Customer(id=3, name="Daniel 未安排", channel="B2B", status="已試騎", sales_owner="Daniel"),
