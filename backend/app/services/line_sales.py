...
不得捏造合格證號、電池容量、續航距離、馬達功率、車輛尺寸重量、保固期限、配送費、看車地址、付款帳號或剩餘精確庫存數量。這些未提供的細節一律說「需由真人客服確認」。
...
PRICE_REPLY = (
    "Be-Bike 全新庫存車每台 NT$12,800，數量有限；多台或團購可另外報價。請提供「所在縣市＋需要數量」，我幫您轉真人客服確認庫存與方案。"
)
...
UNVERIFIED_REPLY = {
    "續航": "續航距離目前需由真人客服確認，我不會用未確認規格推估。想進一步了解的話，請留下所在縣市與預計數量。",
    "電池": "電池容量與詳細規格目前需由真人客服確認，我不會自行猜測。請留下所在縣市與預計數量，我幫您接續詢問。",
...
INVENTORY_REPLY = (
    "Be-Bike 為全新庫存出清，數量有限；剩餘精確庫存需由真人客服確認。"
    "每台 NT$12,800，多台可另行報價。請提供所在縣市與需要數量。"
)
...
def _apply_ai_output_safeguards(text: str) -> str:
    ...
    unverified_numeric_claim = re.search(
        r"(?:合格證號|續航|電池(?:容量)?|馬達(?:功率)?|尺寸|重量|保固(?:期限)?|配送費|運費|付款帳號|匯款帳號|庫存(?:數量)?)"
        r"[^\d\n]{0,12}\d",
        reply,
    )
...
def get_structured_sales_reply(message: str) -> str | None:
    ...
    if any(keyword in compact for keyword in ("續航", "騎多遠", "跑多遠", "幾公里")):
        return UNVERIFIED_REPLY["續航"]
    if any(keyword in compact for keyword in ("電池", "電瓶", "幾安培", "幾伏")):
    ...
    if any(keyword in compact for keyword in ("剩幾台", "剩下幾台", "庫存數量", "精確庫存")):
        return INVENTORY_REPLY
    if any(keyword in compact for keyword in ("全新", "庫存車", "二手")):
        return "本批 Be-Bike 是全新庫存出清車，每台 NT$12,800，數量有限。請提供所在縣市與數量，我幫您確認庫存。"
