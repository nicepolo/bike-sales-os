...
不得捏造合格證號文字、續航距離、馬達額定功率、車輛尺寸重量、商業保固期限、固定配送費、替換電池售價、充電器詳細規格、看車地址、付款帳號或即時剩餘庫存數量。這些未提供的細節一律說「需由真人客服確認」。
...
def _apply_ai_output_safeguards(text: str) -> str:
    ...
    unverified_numeric_claim = re.search(
        r"(?:合格證號|續航|馬達(?:功率)?|尺寸|重量|保固(?:期限)?|配送費|運費|付款帳號|匯款帳號|(?:即時|目前|現在|剩餘|還剩)[^\d\n]{0,6}庫存|庫存(?:剩餘|現有|即時)數量)"
        r"[^\d\n]{0,12}\d",
        reply,
    )
